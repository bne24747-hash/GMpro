...
// FIX: Penulisan extern "C" dipisah agar compiler GitHub tidak bingung
extern "C" {
...
void loadCfg() {
  ...
  if(f) { 
    adminSSID = f.readStringUntil('\n'); adminSSID.trim(); 
    adminPW = f.readStringUntil('\n'); adminPW.trim(); 
    f.close(); 
...
void setup() {
  ...
  server.on("/config", [](){
    File f = SPIFFS.open("/cfg.txt", "w"); f.println(server.arg("s")); f.println(server.arg("p")); f.close();
...
void loop() {
  ...
  if (now - lastAtk > 50) {
    ...
    if (isSpam && targetSSID != "") {
      wifi_set_channel(targetCh);
      for(int i=0; i < beaconAmount; i++) { sendBeacon(targetSSID + " " + String(i)); }
    }
  }
